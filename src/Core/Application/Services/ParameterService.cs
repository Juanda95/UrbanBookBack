        ...
        public async Task<Response<ParameterDTOResponse>> CreateAsync(ParametersDTORequest request)
        {
            try
            {
                using (_unitOfWork)
                {
                    ...
                    return new Response<ParameterDTOResponse>(parameterResponse, "El par�metro se cre� con �xito", HttpStatusCode.Created);
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Ocurri� un error en el proceso de creaci�n del par�metro: {ex.Message}");
            }
        }
        ...
        public async Task<Response<bool>> DeleteAsync(int Id)
        {
            try
            {
                using (_unitOfWork)
                {
                    ...
                    if (parameter == null)
                    {
                        return new Response<bool>($"No se encontr� ning�n par�metro con el ID {Id}", HttpStatusCode.NotFound);
                    }

                    parameterRepository.Delete(parameter);
                    await _unitOfWork.SaveChangesAsync();
                    return new Response<bool>(true, "Par�metro eliminado correctamente");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Ocurri� un error al eliminar el par�metro: {ex.Message}");
            }
        }
        ...
        public async Task<Response<List<ParameterDTOResponse>>> GetAll()
        {
            try
            {
                using (_unitOfWork)
                {
                    ...
                    if (!parameters.Any())
                    {
                        return new Response<List<ParameterDTOResponse>>("No se encontraron par�metros", HttpStatusCode.NotFound);
                    }

                    var parametersResponse = parameters.Select(p => _mapper.Map<ParameterDTOResponse>(p)).ToList();
                    return new Response<List<ParameterDTOResponse>>(parametersResponse, "Par�metros obtenidos con �xito");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Ocurri� un error en la consulta de par�metros: {ex.Message}");
            }
        }
        ...
        public async Task<Response<ParameterDTOResponse>> GetById(int Id)
        {
            try
            {
                using (_unitOfWork)
                {
                    ...
                    if (parameter == null)
                    {
                        return new Response<ParameterDTOResponse>($"No se encontr� informaci�n para el ID {Id}", HttpStatusCode.NotFound);
                    }

                    var parameterResponse = _mapper.Map<ParameterDTOResponse>(parameter);
                    return new Response<ParameterDTOResponse>(parameterResponse, "Par�metro obtenido con �xito");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Ocurri� un error al obtener el par�metro: {ex.Message}");
            }
        }
        ...
        public async Task<Response<bool>> UpdateAsync(ParameterDTOUpdateRequest updateRequest)
        {
            try
            {
                using (_unitOfWork)
                {
                    ...
                    {
                        return new Response<bool>($"No se encontr� ning�n par�metro con el ID {updateRequest.IdParameter}", HttpStatusCode.NotFound);
                    }

                    if (updateRequest.Values == null || !updateRequest.Values.Any())
                    {
                        return new Response<bool>("Debe proporcionar al menos un valor para actualizar el par�metro", HttpStatusCode.BadRequest);
                    }
                    ...
                    // Elimina los valores que no est�n en el updateRequest
                    ...
                    await _unitOfWork.SaveChangesAsync();
                    return new Response<bool>(true, "Par�metro actualizado correctamente");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"Ocurri� un error durante la actualizaci�n del par�metro: {ex.Message}");
            }
        }
    }
}
